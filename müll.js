...
* Weiter verwendet zur Lösung von Aufgabe 4
...
// Function which sets the bus-stop-stations on the map together with their names and distances
...
map.on("draw:created", function (e) {
    ...
    if (type == "rectangle") {
        ...
        selectBusStop(layer.toGeoJSON())
    }
    drawnItems.addLayer(layer)

});

// Function to select bus stop stations only selected area
function selectBusStop(polygon) {
    busItems.clearLayers()
    for (let i = 0; i < results.length; i++) {
        const coords = results[i].coordinates
        var pt = turf.point(coords);
        if (turf.booleanPointInPolygon(pt, polygon)) {
            let marker = L.marker([coords[1], coords[0]])
                .bindPopup(results[i].name + '<br>' + "Entfernung: " + results[i].distance + "m")
...
function calculateResults(point, busStops) {
    results = sortByDistance(point, busStops);
